...
void fileAfterFileRun(std::string directoryName)
{
	try
	{
		...
		header << "polno ime; Kratko ime; Naslov; Naselje; Pošta; Organizacijska oblika; "
			<< "Število zaposlenih; Telefon; Fax; Gsm; Splet; Matična številka; Davčna številka; "
			<< "ID številka za DDV; Transakcijski račun; Ustanovitelji; Datum vpisa pri registrskem organu; "
...
std::vector<std::string> parseFile(std::vector<std::string>& details, std::string& file)
{
	try
	{
		...
		details.push_back(findDetail(file, "Matična številka:", "</td>", 38));

		//finding tax code
		details.push_back(findDetail(file, "Davčna številka:", "</td>", 37));

		//finding ID code for VAT
		details.push_back(findDetail(file, "ID številka za DDV:", "</td>", 41));

		//finding transaction numbers
		std::string::size_type detailPosition = file.find("Transakcijski račun:");
		std::string detail;
		...
		for(int i = 0; i <= number; ++i)
		{
			...
			if(i == 0)
				details.push_back(findDetail(file, "Transakcijski račun:", "<br />", 29));
			else
